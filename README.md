...
| 写回 | `EXCEL_LOCK_MAX_WAIT` | Excel 被占用时的等待上限，**单位秒**。默认 `300`，`0`=无限。中途用 Excel 打开查看导致文件锁定时，控制台只告警一次、任务继续跑；关闭后自动恢复。任务跑完仍被占用则阻塞等待至写成功再退出；超时仍占用则另存 `<Excel名>.pending.xlsx` 兜底 |
...
**中途用 Excel 打开查看（文件被占用）怎么办**

流水线在后台按 `EXCEL_FLUSH_INTERVAL` 周期落盘；若此时你用 Excel 打开了同一个 `.xlsx`，文件被独占锁定，落盘会失败。处理策略（双端一致）：

1. **只告警一次**：首次检测到占用打印一行 `⚠️ Excel 文件被占用…将每 Ns 重试`，之后保持静默，不再每 3 秒刷屏。
2. **任务继续跑**：落盘在独立的后台定时器/线程，不影响主流程；脏数据一直保留在内存，不会丢。
3. **自动恢复**：你关闭 Excel 查看窗口后，下一次周期落盘自动写成功，并打印 `✅ Excel 已恢复写入`。
4. **收尾不丢数据**：若所有任务跑完文件仍然被占用，进程会**阻塞等待**直到写成功再退出（不再像旧版那样静默丢数据）。等待上限由 `EXCEL_LOCK_MAX_WAIT`（秒，默认 `300`，`0`=无限）控制；超过上限仍占用，则把全部改动另存为 `<Excel名>.pending.xlsx`（该文件未被锁定），你可打开后把值复制回原表。

> 缺失列（如 `content` / `keywords`）无需手动建：首次写入时自动在末列创建该列并写表头，后续命中即复用。
